...
if($music_list == false){
    ...
    header('Location: index.php?id=1');  //不存在音乐就跳转
}
// echo '<iframe src="http://106.13.115.15:85/html/canvas/jiazai/'.rand(1,3).'.html" style="z-index: 100000000000;border:0px;width:100%;height:100%；position:fixed;" id="jiazai"></iframe>';  //加载动画
...
<head>
    ...
    <meta itemprop="image" content="<?php if($music_list['small_image']){
        ...
        echo 'http://106.13.115.15:85/image/music/qq.jpg';
    }
...
<body id="body" onunload=fclose() onload=fload() onbeforeunload=bfunload()>
...
<script type="text/javascript">
    ...
    window.onload=function(){
        ...
        document.cookie="jiazai_time="+new Date().getTime(); //加载完毕时间
        playPause();   //开启音乐
...
<script>  //离开或者刷新页面
    var opentime = new Date().getTime();  //打开页面时间
    //document.cookie="opentime="+new Date().getTime();
    var s = "test";
    function fclose(){


        if(get_cookie("jiazai_time")){  //
            jiazai_time = get_cookie("jiazai_time")-parseInt(opentime);  //加载时间
            delCookie(jiazai_time);	 //删除cookie

        }else{
            var jiazai_time='0';
        }
        //var zaixian_time = parseInt(new Date().getTime()) - parseInt(get_cookie("opentime"));
        var zaixian_time = parseInt(new Date().getTime()) - parseInt(opentime);
        $.ajax({
            url:'pv.php',
            data:'zaixian_time='+zaixian_time+'&jiazai_time='+jiazai_time+'&id='+<?php echo $id ?>,
            type:'post',
            dataType:'json',
            async: false,
            success:function(){

                //document.cookie='zaitime='+zaixian_time;
            }
        })
    ...
    function fload(){   document.cookie="popped3=yes3";}
    function bfunload(){   s = "no";}
    ...
    //使用方式：
    //console.log(get_cookie("zaitime"));console.log(get_cookie("opentime"));
</script>
<!-- qq链接 -->
